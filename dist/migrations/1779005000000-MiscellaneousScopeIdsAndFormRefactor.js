...
exports.MiscellaneousScopeIdsAndFormRefactor1779005000000 = void 0;
class MiscellaneousScopeIdsAndFormRefactor1779005000000 {
    ...
    `);
        const oc = new Set(ownerCols.map((r) => r.column_name));
        if (!oc.has('role')) {
    ...
    `);
        const mc = new Set(miscCols.map((r) => r.column_name));
        if (!mc.has('scope_ids')) {
    ...
    `);
        const fc = new Set(formCols.map((r) => r.column_name));
...
exports.MiscellaneousScopeIdsAndFormRefactor1779005000000 = MiscellaneousScopeIdsAndFormRefactor1779005000000;
