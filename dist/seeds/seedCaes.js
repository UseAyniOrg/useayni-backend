...
async function seedCaes() {
    ...
    try {
        // Buscar estados
        const states = await db_1.AppDataBase.query('SELECT id, name, uf FROM states');
        // Criar CAEs para cada estado
        for (const state of states) {
            const caeName = `CAE ${state.name}`;
            // Verificar se já existe
            const existing = await db_1.AppDataBase.query('SELECT id FROM caes WHERE name = $1', [caeName]);
            if (existing.length === 0) {
                await db_1.AppDataBase.query('INSERT INTO caes (name, state_id) VALUES ($1, $2)', [caeName, state.id]);
