...
export class MiscellaneousParticipationAndScopeRules1779005000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    const rows = await queryRunner.query(`
      SELECT column_name FROM information_schema.columns
      WHERE table_schema = 'public' AND table_name = 'miscellaneous'
    `);
    const mc = new Set(rows.map((r: any) => r.column_name));
    ...
    `);
    const oc = new Set(ownerRows.map((r: any) => r.column_name));
    if (!oc.has('role')) {
    ...
    `);
    const fc = new Set(formRows.map((r: any) => r.column_name));
